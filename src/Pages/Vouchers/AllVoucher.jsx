...
const AllVoucher = () => {
    ...
    useEffect(() => {
        const fetchVouchers = async () => {
            try {
                const response = await axios.get('http://localhost:6842/api/v1/vouchers');  // Adjust the endpoint as needed
    ...
    const markActive = async (id) => {
        try {
            await axios.put(`http://localhost:6842/api/v1/vouchers/activateVoucher/${id}`);
    ...
    const markInActive = async (id) => {
        try {
            await axios.put(`http://localhost:6842/api/v1/vouchers/deactivateVoucher/${id}`);
            // Update the local state to reflect the change
    ...
    const markDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:6842/api/v1/vouchers/deleteVoucher/${id}`);
